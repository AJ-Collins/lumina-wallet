...
import AboutProfile from '../components/wallet/settings/AboutProfile';
import ChangePassword from '../components/wallet/settings/ChangePassword';
import RecoveryPhraseReveal from '../components/wallet/settings/RecoveryPhraseReveal';
import NotificationSettings from '../components/wallet/settings/NotificationSettings';

export default function Settings() {
   const { user } = useAuth();
   const [activeView, setActiveView] = useState('main'); // 'main', 'profile', 'notifications', 'password', 'phrase'

   const displayName = user?.username?.trim() || 'Lumina Member';
      ...
      <div className="max-w-2xl mx-auto w-full animate-in fade-in duration-300 pb-24 md:pb-0">
         <div className="bg-card border border-border rounded-2xl shadow-sm p-6 md:p-8 mb-6 flex flex-col md:flex-row items-center gap-6 md:gap-8">
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
               <h2 className="text-2xl font-black text-foreground tracking-tight mb-1" title={displayName}>
                  {truncatedName}
               </h2>
